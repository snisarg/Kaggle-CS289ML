...
annot_bids = pandas.merge(left=train, right=bids, how='left', left_on='bidder_id', right_on='bidder_id')
...
# auction_counts.loc[math.isnan(float(auction_counts['auction_count'])),'auction_count'] = 0.0
...
device_counts = inner_join[['bidder_id', 'device_count', 'outcome']].sort_values(by=['device_count'], ascending=False).drop_duplicates()
# print device_counts
...
ip_counts = inner_join[['bidder_id', 'ip_count', 'outcome']].sort_values(by=['ip_count'], ascending=False).drop_duplicates()
# print ip_counts
...
country_counts = inner_join[['bidder_id', 'country_count', 'outcome']].sort_values(by=['country_count'], ascending=False).drop_duplicates()
...
# print edev
# edev.to_csv('edev.csv',index=False)
...
# print eip
# eip.to_csv('eip.csv',index=False)
...
#     lambda s: s.count()
# )
# ecntry['n_c'] = ecntry[['bid_id', 'bidder_id', 'country']].drop_duplicates().groupby(by=['bidder_id', 'country'], as_index=False)['bid_id'].transform(
#     lambda s: s.count()
# )
# print "n_c done"
...
# print ecntry
# ecntry.to_csv('ecntry.csv',index=False)